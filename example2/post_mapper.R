...
emit <- function(lines){
  apply(lines, 1, function(x){
    ...
    country <- as.character(x[6])
    if(amount > 200 && country != "Norge"){
...
(function() {
  ...
  names <- c("id", "order.date", "delivery.date", "sender.name","sender.address","sender.country","recipient.name", "recipient.address", "declared.value")
